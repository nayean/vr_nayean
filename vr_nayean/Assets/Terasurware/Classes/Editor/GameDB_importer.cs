...
public class GameDB_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/WorkDB/GameDB.xlsx";
	private static readonly string exportPath = "Assets/WorkDB/GameDB.asset";
		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(1); p.objuectid = (cell == null ? "" : cell.StringCellValue);
